...
# Real fixtures that EXECUTE in the container: a "bad" case exits nonzero, a "good" case exits 0.
_BAD = b"raise SystemExit(1)\n"
_GOOD = b"raise SystemExit(0)\n"
...
class AcceptanceAnchorOnRealPodmanTests(unittest.TestCase):
    def test_two_sided_anchor_signs_a_real_accepted_receipt(self) -> None:
        visible = CalibrationSet(
            known_good=(Fixture("vg", FixtureLabel.KNOWN_GOOD, _GOOD),),
            known_bad=(Fixture("vb", FixtureLabel.KNOWN_BAD, _BAD),),
        )
        holdout = BlindHoldoutStore(Path(tempfile.mkdtemp(prefix="mv-hold-oci-")) / "h.db")
        holdout.append(Fixture("hb", FixtureLabel.KNOWN_BAD, _BAD),
                       holdout_key=_HOLDOUT_KEY, approval=_cal_gov("cg1", "cg2"))
        holdout.append(Fixture("hg", FixtureLabel.KNOWN_GOOD, _GOOD),
                       holdout_key=_HOLDOUT_KEY, approval=_cal_gov("cg1", "cg2"))
        ...
        report = run_acceptance_anchor(
            ...
            fp_happy_detector=_AlwaysFail(), visible_set=visible, blind_holdout_store=holdout,
            holdout_key=_HOLDOUT_KEY, signer_key=_SIGNER_KEY, signer_principal="cal-gov-1",
            signer_approval=_cal_gov("cal-gov-1"),
            sandbox_config_hash=sandbox_config_digest(backend="oci", image=IMAGE,
                                                      isolation="hermetic"),
        ...
        # the receipt carries the REAL sandbox config hash (not the NoOp one).
        self.assertEqual(report.sandbox_config_hash,
                         sandbox_config_digest(backend="oci", image=IMAGE, isolation="hermetic"))
